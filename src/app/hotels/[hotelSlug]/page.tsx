      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_360px]">
        <div>
          ...
          <section id="rooms" className="mt-14 border-t border-[var(--line)] pt-12">
            ...
            {unit && <div className="surface-3d surface-3d-lift mt-6 p-6"><div className="flex justify-between gap-4"><div><strong className="font-serif text-xl text-[var(--ink)]">{unit.name}</strong><p className="mt-2 text-sm text-[var(--muted)]">Up to {unit.maxGuests} guests · {unit.bedrooms} bedrooms · {unit.baths} bathrooms</p></div><strong className="text-[var(--gold-deep)]">RWF {unit.basePriceRwf.toLocaleString()}</strong></div></div>}
          </section>
          <section id="amenities" className="mt-14 border-t border-[var(--line)] pt-12">
            <p className="eyebrow">Amenities</p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">{hotel.amenities.map((item) => <span key={item} className="flex gap-3 text-[var(--muted)]"><Check size={18} className="text-[var(--gold-deep)]" />{item}</span>)}</div>
          </section>
          <section id="location" className="mt-14 border-t border-[var(--line)] pt-12">
            <p className="eyebrow">Location</p>
            <div className="surface-3d mt-6 grid min-h-64 place-items-center bg-[var(--cream)]">
              <div className="surface-3d p-6 text-center">
                ...
                {hotel.location.lat && hotel.location.lng && <a className="button-3d mt-4 inline-block bg-[var(--ink)] px-5 py-2 text-xs uppercase tracking-wider text-white" href={`https://www.google.com/maps?q=${hotel.location.lat},${hotel.location.lng}`} target="_blank">Open map</a>}
              </div>
            </div>
          </section>
        </div>
