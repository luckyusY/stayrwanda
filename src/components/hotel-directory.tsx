...
export function HotelDirectory({ hotels, eyebrow, title, copy }: { hotels: Hotel[]; eyebrow: string; title: string; copy: string }) {
  return <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
    ...
    {!hotels.length && <div className="mt-10 border border-[var(--line)] bg-[var(--cream)] p-10 text-center"><h2 className="font-serif text-2xl text-[var(--ink)]">More places are being prepared</h2><p className="mt-2 text-sm text-[var(--muted)]">Our local team is reviewing new profiles now.</p></div>}
  </section>;
}
