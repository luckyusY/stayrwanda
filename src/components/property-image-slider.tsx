...
export function PropertyImageSlider({ images, alt, href, sizes = "(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw", aspect = "aspect-[4/3]", priority = false, max = 8 }: Props) {
  ...
  return (
    <div className={`group/slider relative w-full ${aspect} overflow-hidden bg-[var(--cream-2)]`}>
      ...
      <button type="button" aria-label="Previous photo" onClick={(event) => { event.preventDefault(); goTo(index - 1); }} disabled={index === 0} className="absolute left-3 top-1/2 z-20 grid size-11 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[var(--ink)] shadow transition hover:bg-white disabled:pointer-events-none disabled:opacity-0 sm:size-9 sm:opacity-0 sm:group-hover/slider:opacity-100"><ChevronLeft size={18} /></button>
      <button type="button" aria-label="Next photo" onClick={(event) => { event.preventDefault(); goTo(index + 1); }} disabled={index === slides.length - 1} className="absolute right-3 top-1/2 z-20 grid size-11 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[var(--ink)] shadow transition hover:bg-white disabled:pointer-events-none disabled:opacity-0 sm:size-9 sm:opacity-0 sm:group-hover/slider:opacity-100"><ChevronRight size={18} /></button>
