...
export async function createInventoryBackedBooking(raw: unknown, idempotencyKey: string, actor?: AppIdentity | null) {
  ...
  if (isSeed) {
    ...
    const record = {
      ...
      status: "pending",
      propertySnapshot: { name: seedHotel?.title || "StayRwanda property" },
      allocationState: "held",
      allocatedDates: dates,
      pricingSnapshot: { nightlyRwf: finalNightly, subtotalRwf: finalNightly * dates.length * input.quantity, cleaningFeeRwf: 0, serviceFeeRwf: 0, totalRwf: finalNightly * dates.length * input.quantity, currency: input.currency },
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const record = {
        ...
        message: input.message,
        status: "pending",
        propertySnapshot: { name: String(hotel.name || "StayRwanda property") },
        allocationState: "held",
        allocatedDates: dates,
        pricingSnapshot: { nightlyRwf: nightly, subtotalRwf: nightly * dates.length * input.quantity, cleaningFeeRwf: 0, serviceFeeRwf: 0, totalRwf: nightly * dates.length * input.quantity, currency: input.currency },
